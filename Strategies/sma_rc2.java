...
public class sma_rc2 implements IStrategy {
    ...
    public double thresholdPips = timePeriodSlow * 0.0; /* 40% or 14-18 pips */
    ...
    public void onStart(IContext context) throws JFException {
        ...
        if (chart != null && engine.getType() == IEngine.Type.TEST) {
            chart.addIndicator(indicators.getIndicator("MA"), new Object[] {timePeriodSlow, maTypeSlow.ordinal()});
        }
    ...
    public void onTick(Instrument instrument, ITick tick) throws JFException {
        ...
        if (Double.isNaN(mas[PREV]))
            return;

        // Buy/Long
        if (mas[PREV] > mas[PREV-1] && tick.getBid() > mas[PREV-1]) {
            if (order == null || !order.isLong()) {
                closeOrder(order);

                if (getRoundedPips(getPricePips(instrument, Math.abs(mas[PREV] - mas[PREV-1]))) > thresholdPips) {
